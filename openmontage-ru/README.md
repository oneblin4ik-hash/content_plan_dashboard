...
```json
{
  ...
  "captionTextShadow": "0 0 14px rgba(0,0,0,0.95), 0 0 30px rgba(0,0,0,0.85), 0 0 60px rgba(0,0,0,0.6)",
  "captionDimUpcoming": false,
  "captionInstantIn": true
}
```

`captionTextShadow` здесь задаёт не смещённую тень, а **тёмное свечение вокруг
букв**: три слоя размытия без смещения. Без него белый текст на светлом кадре
(например, на белой футболке) разваливается и не читается. Три слоя нужны,
чтобы ореол спадал плавно: узкий и плотный у самых букв, широкий и слабый по
краю. Одним слоём получается либо ободок, либо грязное пятно.
